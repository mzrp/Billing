...
namespace RPNAVConnect
{
    ...
    public class RPNAVConnectWS : System.Web.Services.WebService
    {
        ...
        /*
        public string SiteCode = "da10d62d40024fbfb960d378d";
        public string ApiUser = "TL2rpconsult";
        public string ApiPw = "TL2rpconsult";
        */
        ...
        {
            return "SiteCode: " + SiteCode + " <br />ApiUser: " + ApiUser + "<br />ApiPw contains " + ApiPw.Length.ToString() + " chars";
        }
        ...
        public strRPNAVConnectWS GetTimeLogCustomersData()
        {
            ...
            client.Endpoint.Binding.OpenTimeout = new TimeSpan(0, 15, 0);
            client.Endpoint.Binding.CloseTimeout = new TimeSpan(0, 15, 0);
            client.Endpoint.Binding.SendTimeout = new TimeSpan(0, 15, 0);
            client.Endpoint.Binding.ReceiveTimeout = new TimeSpan(0, 15, 0);
            ...
            stRPNAVConnectWS.sResultInvoices = "n/a";
            stRPNAVConnectWS.sResultInvoiceLines = "/n/a";
        ...
        {
            string sResultDesc = "";
            sResultDesc += "Service start: " + DateTime.Now.ToString() + "<br />";
            ...
            try
            {
                sResultDesc += "InvoicesRaw start: " + DateTime.Now.ToString() + "<br />";

                // get invoices
                if ((bStartDate == true) && (bEndDate == true))
                {
                    xmlNodeInvDetails = client.GetInvoicesRaw(SiteCode, ApiUser, ApiPw, 0, -1, iInvoiceStatus, (DateTime?)dtStartDate, (DateTime?)dtEndDate);
                }
                else
                {
                    xmlNodeInvDetails = client.GetInvoicesRaw(SiteCode, ApiUser, ApiPw, 0, -1, iInvoiceStatus, null, null);
                }
                ...
                try
                {
                    ...
                    if (xnListInvDetails.Count > 0)
                    {
                        ...
                        sResultDesc += "InvoicesRaw finished: " + DateTime.Now.ToString() + "<br />";
                        ...
                        foreach (System.Xml.XmlNode xnInvDetails in xnListInvDetails)
                        {
                            ...
                            if (((sType == "1") || (sType == "2")) && (sInvoiceNo != "n/a"))
                            {
                                ...
                                try
                                {
                                    iCustomerID = Convert.ToInt32(sCustomerID);
                                }
                                catch (Exception ex)
                                {
                                    ex.ToString();
                                ...
                                if (iInvoiceIDInv != -1)
                                {
                                    if (iCustomerID != -1)
                                    {
                                        ...
                                        try
                                        {
                                            sResultDesc += "InvoicesLinesRaw start (" + iInvoiceIDInv.ToString() + "): " + DateTime.Now.ToString() + "<br />";
                                            ...
                                            try
                                            {
                                                ...
                                                sResultDesc += "InvoicesLinesRaw finished (" + iInvoiceIDInv.ToString() + "): " + DateTime.Now.ToString() + "<br />";
                                                ...
                                                foreach (System.Xml.XmlNode xnInv in xnListInv)
                                                {
                                                    ...
                                                    try { sProjectID = xnInv["tlp:ProjectID"].InnerText; }
                                                    catch { }

                                                    string sProjectName = "n/a";
                                                    try { sProjectName = xnInv["tlp:ProjectName"].InnerText; }
                                                    catch { }

                                                    string sProductNo = "n/a";
                                                    try { sProductNo = xnInv["tlp:ProductNo"].InnerText; }
                                                    catch { }
                                                    ...
                                                    try
                                                    {
                                                        ...
                                                        if (iProjectId != -1)
                                                        {
                                                            sResultDesc += "ProjectsRaw start (" + iProjectId.ToString() + "): " + DateTime.Now.ToString() + "<br />";
                                                            ...
                                                            try
                                                            {
                                                                ...
                                                                sResultDesc += "ProjectsRaw finished (" + iProjectId.ToString() + "): " + DateTime.Now.ToString() + "<br />";
                                            ...
                                            sResultDesc += "InvoiceLinesRaw finished (" + iInvoiceIDInv.ToString() + "): " + DateTime.Now.ToString() + "<br />";
            ...
            sResultDesc += "Service finished: " + DateTime.Now.ToString() + "<br />";
        ...
        public strRPNAVConnectWS GetTimeLogAllInvoicesNoProjectsData(string sInvoiceStatus, string sStartMonth, string sStartYear, string sEndMonth, string sEndYear)
        {
            ...
            if (iInvoicesCount > 0)
            {
                ...
                try
                {
                    sResultDesc += "InvoiceLinesRaw start: " + DateTime.Now.ToString() + "<br />";

                    // get invoice lines
                    if ((bStartDate == true) && (bEndDate == true))
                    {
                        xmlNodeInv = client.GetInvoiceLinesRaw(SiteCode, ApiUser, ApiPw, 0, -1, 0, -1, (DateTime?)dtStartDate, (DateTime?)dtEndDate);
                    }
                    else
                    {
                        xmlNodeInv = client.GetInvoiceLinesRaw(SiteCode, ApiUser, ApiPw, 0, -1, 0, -1, null, null);
                    }
                    ...
                    try
                    {
                        ...
                        foreach (System.Xml.XmlNode xnInv in xnListInv)
                        {
                            ...
                            // invoice lines
                            sResultInvoiceLines += "\"" + sInvLineID.Replace(",", "╬") + "\",\"" + sInvoiceID.Replace(",", "╬") + "\",\"" + sInvoiceLineNo.Replace(",", "╬") + "\",\"" + sDate.Replace(",", "╬") + "\",\"" + sText.Replace(",", "╬") + "\",\"" + sQuantity.Replace(",", "╬") + "\",\"" + sRate.Replace(",", "╬") + "\",\"" + sRateSystemCurrency.Replace(",", "╬") + "\",\"" + "Amount: " + sAmount.Replace(",", "╬") + "\",\"" + sAmountSystemCurrency.Replace(",", "╬") + "\",\"" + sProjectID.Replace(",", "╬") + "\",\"" + sVAT.Replace(",", "╬") + "\",\"" + sDiscount.Replace(",", "╬") + "\",\"" + sUnitType.Replace(",", "╬") + "\",\"" + sProjectName.Replace(",", "╬") + "\",\"" + sInvoiceLineNo.Replace(",", "╬") + "\",\"" + sProductNo.Replace(",", "╬") + "\",\"" + sProjectName.Replace(",", "╬") + "\"" + " <br /><br />";
                        }
                    }
                    catch (Exception ex)
                    {
                    ...
                    sResultDesc += "InvoiceLinesRaw finished: " + DateTime.Now.ToString() + "<br />";
                }
                catch (Exception ex)
                {
                    sResultDesc += ex.ToString();
